...
- **stage**: takes the lowest-numbered article in the vault `Articles/Posts/` folder that is not yet queued or published, runs a headless Claude pass on it (image under the standing image rule, fact-check, grader, read-aloud, a listing summary in Russell’s voice, theme), verifies the result with deterministic gates, and copies it into `src/content/queue/`. Two articles per scheduled run (`stage --all` does every waiting article), never more than ten queued ahead. A failure writes a `stage_note` (with the exact gate that failed) into the vault file and sends a notification that opens the file in Obsidian when clicked. An article gets two tries; after that it is parked with `hold: true` for Russell, and the pipeline moves on. Clear `hold`, `stage_note` and `stage_attempts` to put it back in.
